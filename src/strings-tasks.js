...
function removeFirstOccurrences(str, value) {
  ...
  const end = str.slice(str.indexOf(value) + value.length);
  return str.indexOf(value) > -1 ? start.concat(end) : str;
}
...
function removeLastOccurrences(str, value) {
  ...
  const end = str.slice(str.lastIndexOf(value) + value.length);
  return str.lastIndexOf(value) > -1 ? start.concat(end) : str;
}
...
/**
 * Returns a string in reverse order..0
 *
 * @param {string} str - The input string.
...
function countVowels(str) {
  ...
  return str
    .split('')
    .reduce((acc, e) => acc + (vowels.search(e) > -1 ? 1 : 0), 0);
}
...
function isPalindrome(str) {
  const s = str.replace(/[?!,]/g, '').toLowerCase().split(' ').join('');
...
function encodeToRot13(str) {
  ...
  return str
    ...
    .reduce(
      (result, el) =>
        result + (abc.indexOf(el) > -1 ? ROT13[abc.indexOf(el)] : el),
      ''
    );
}
...
function getCardId(value) {
  ...
  return kardsDesk.findIndex((el) => el === value);
}
